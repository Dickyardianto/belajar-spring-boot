...
public class AppUserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        return appUserRepo.findByEmail(email).orElseThrow(() -> new UsernameNotFoundException(String.format("user with email is not found", email)));
    }
    ...
    public AppUser registerAppUser(AppUser user) {
        ...
        if(userExists) {
            throw new RuntimeException(
                String.format("user with email '%s' is found", user.getEmail()));
        }
